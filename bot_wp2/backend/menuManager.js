...
async function loadMenus() {
  try {
    ...
    const [menuRows] = await db.pool.execute('SELECT * FROM menus');
    ...
    const [optionRows] = await db.pool.execute('SELECT * FROM menu_options ORDER BY menu_id, option_id');
...
async function saveMenu(key, data) {
  try {
    ...
    const [existingMenus] = await db.pool.execute(
    ...
    await db.pool.execute('DELETE FROM menu_options WHERE menu_id = ?', [menuId]);
