...
require('./backend/crypto-fix').applyCryptoFix();
...
const calendar = require('./backend/calendar');
const menuManager = require('./backend/menuManager');
const messageRouter = require('./backend/messageRouter');
const { apiStartPostSaleSurvey } = require('./backend/postSaleService');
const { checkUpcomingAppointments } = require('./backend/reminderService');
    ...
    for (const [agent, queue] of Object.entries(queues)) {
      if (queue.length > 1) {
        ...
        for (let i = 1; i < queue.length; i++) {
          const jid = queue[i];
          const pos = i + 1;
          const msg = await db.getCustomText('queue_position_update') || 'Atualização: você agora está na posição {position} da fila.';
      ...
      if (isPending) {
        ...
        if (rows && rows.length > 0) {
          ...
          if (userRows && userRows.length > 0) {
            ...
            // Iniciar a pesquisa
            const postSaleService = require('./backend/postSaleService');
